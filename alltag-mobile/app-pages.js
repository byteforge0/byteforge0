function renderCar(){
  ...
  const monthFuel=data.fuelEntries.filter(f=>sameMonthKey(f.date,key)).reduce((a,b)=>a+b.totalPrice,0),monthOther=data.carExpenses.filter(x=>sameMonthKey(x.date,key)).reduce((a,b)=>a+b.amount,0),leasing=Number(data.settings.leasingMonthly)||0,rem=data.carReminders.filter(r=>!r.done);
...
function renderDay(){
  ...
  return `${pageHeader('Dein Tag',new Intl.DateTimeFormat('de-DE',{month:'long',year:'numeric'}).format(new Date()))}
