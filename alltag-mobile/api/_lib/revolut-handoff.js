...
function key() {
  return crypto.createHash('sha256').update(getPrivateKey()).update('alltag-revolut-handoff-v1').digest();
}
...
export function createHandoffToken(payload) {
  ...
  return `AR1.${iv.toString('base64url')}.${tag.toString('base64url')}.${body.toString('base64url')}`;
}

export function readHandoffToken(token) {
  const [version, ivRaw, tagRaw, bodyRaw] = String(token || '').trim().split('.');
  if (version !== 'AR1' || !ivRaw || !tagRaw || !bodyRaw) throw new Error('invalid_handoff');
