...
export default async function handler(req, res) {
  if (String(req.query?.transactions || '') === '1') {
    return handleBankTransactions(req, res, { cookieName: 'alltag_c24', provider: 'c24' });
  }
  if (req.method !== 'GET') return res.status(405).json({ error: 'method_not_allowed' });
  res.setHeader('Cache-Control', 'no-store');
  try {
    const auth = verifyCookiePayload(getCookie(req, 'alltag_c24'));
    ...
    if (!ok.length) {
      ...
      if (expired) res.setHeader('Set-Cookie', cookie('alltag_c24', '', 0));
    ...
    console.error('c24-balance', e?.status, e?.body || e?.message);
    return res.status(503).json({ connected: false, error: 'c24_unavailable' });
  }
}
