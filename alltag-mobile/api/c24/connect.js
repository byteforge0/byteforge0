...
const CALLBACK = 'https://alltag-mobile.vercel.app/api/c24/callback';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const c24 = banks.find(b => /^c24$/i.test(b.name)) || banks.find(b => /c24/i.test(b.name));
    if (!c24) return res.status(503).send('C24 ist bei Enable Banking für Deutschland gerade nicht verfügbar.');
    ...
    const max = Number(c24.maximum_consent_validity) || 90 * 24 * 60 * 60;
    ...
    console.error('c24-connect', error?.status, error?.body || error?.message);
    return res.status(503).send('C24-Verbindung konnte nicht gestartet werden.');
  }
}
